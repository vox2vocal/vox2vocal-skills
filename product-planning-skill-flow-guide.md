...
| 화면 (Page) | 페이지 / 플로우 기획 (Page / Flow Plan) | 각 서비스 영역에서 사용자는 어떤 화면과 흐름으로 완료하는가? |
...
- `page-flow-planner`는 관리자 서비스와 사용자 앱 페이지를 별도 섹션으로 작성합니다.
- `page-flow-reviewer`는 페이지/플로우가 TRD로 넘어가기 전에 서비스 분리, 상태, 권한, 연동 누락을 검토합니다.
...
기능 정의를 서비스 영역별 페이지와 사용자 흐름으로 바꿉니다.

사용할 때:

- 기능은 정의됐지만 어떤 화면과 흐름으로 드러날지 불명확할 때
- page inventory, states, navigation이 필요할 때
- 관리자 서비스와 사용자 앱 페이지가 분리되어야 할 때
- 디자인/프론트엔드 작업 전 흐름을 잠가야 할 때

프롬프트:

```text
page-flow-planner 스킬을 사용해서 아래 검토된 기능 정의서를 페이지/플로우 기획으로 바꿔줘.
...
각 페이지마다 서비스 영역, 담당 역할, 목적, 주요 행동,
필요한 데이터, 빈 상태, 로딩 상태, 에러 상태, 권한 상태,
이동 경로, 성공 신호를 정리해줘.
공통/연동 플로우가 있으면 트리거, 출발 서비스, 도착 서비스,
전달 데이터, 실패/권한 상태를 별도로 정리해줘.
페이지 x 기능 x 서비스 매트릭스도 포함해줘.
```

다음으로 넘어가는 기준:

- 모든 MVP 기능이 페이지 또는 흐름에 매핑된다.
- 관리자 서비스와 사용자 앱 페이지가 구분된다.
- happy path뿐 아니라 empty/loading/error/permission state가 있다.
- 데이터 요구사항이 드러난다.

### 12. page-flow-reviewer

페이지/플로우 기획이 TRD bridge로 넘어갈 만큼 안정적인지 비판적으로 검토합니다.

사용할 때:

- page-flow-planner 산출물 작성 직후
- 관리자 서비스와 사용자 앱 페이지가 섞였는지 확인해야 할 때
- 페이지-기능 커버리지, 상태, 권한, 연동 흐름을 점검해야 할 때

프롬프트:

```text
page-flow-reviewer 스킬을 사용해서 아래 페이지/플로우 기획을 비판적으로 리뷰해줘.
...
- `Go` 또는 작은 수정 후 진행 가능한 상태다.
- 페이지/플로우 blocker가 있으면 `page-flow-planner`로 돌아간다.
...
| 페이지 상태나 사용자 흐름이 비어 있음 | page-flow-planner |
| 페이지/플로우 리뷰에서 blocker가 발견됨 | page-flow-planner |
...
- 페이지 기획이 관리자 서비스와 사용자 앱을 구분하고 happy path와 예외 상태를 모두 다루는가?
- 페이지/플로우 리뷰가 서비스 분리, 페이지-기능 커버리지, 상태/권한/연동 누락을 잡았는가?
