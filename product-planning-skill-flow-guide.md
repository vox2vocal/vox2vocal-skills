...
- 산출물의 큰 제목과 주요 기술/도메인 용어는 항상 `한글 (English)` 형식으로 작성합니다.
- 기술 객체, 데이터 객체, 이벤트, 패턴 이름은 한글 설명과 원문 식별자를 함께 씁니다. 예: `감사 이벤트 (AuditEvent)`, `접근 결정 로그 (AccessDecisionLog)`, `트랜잭션 아웃박스 (TransactionalOutbox)`.
...
산출물의 큰 제목과 주요 기술/도메인 용어는 한글 (English) 형식으로 작성해줘.
